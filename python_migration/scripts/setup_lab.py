...
def phase_create_vms(definition, config, defaults, json_file, keep):
    ...
    for vm_name in definition.get("nodes", {}):
        ...
        try:
            _, keep_virt_srv = lc.locate_kvm_host(definition, vm_name, config)
        except SystemExit:
            keep_virt_srv = None
        if keep and keep_virt_srv and lc.vm_is_reusable(keep_virt_srv, vm_name, env.get("mymac", ""), env.get("myip", "")):
...
def phase_reboot_and_wait_kept_nodes(definition, config, keep):
    ...
    for vm_name, node_cfg in definition.get("nodes", {}).items():
        ...
        _, virt_srv = lc.locate_kvm_host(definition, vm_name, config)
        lc.reboot_vm(virt_srv, vm_name)
    lc._level -= 1
...
def _install_k8s_on_cluster(definition, clu_name, clu_type, clu_cfg):
    ...
    for vm_name, node_cfg in definition.get("nodes", {}).items():
        if node_cfg.get("kcluster") != clu_name:
            continue
        if clu_type == "k3s":
            token, rancher1_ip = k8s.setup_k3s(
                vm_name, clu_name, clu_cfg.get("clu_rel", ""), clu_cfg.get("mydomain", ""),
                token=token, rancher1_ip=rancher1_ip,
            )
        elif clu_type == "rke2":
            token, rancher1_ip = k8s.setup_rke2(
                vm_name, vm_name, clu_name, clu_type, clu_cfg.get("clu_rel", ""),
                clu_cfg.get("mydomain", ""),
                node_type=node_cfg.get("INSTALL_RKE2_TYPE", "server"),
                token=token, rancher1_ip=rancher1_ip,
            )
        else:
            lc.die("setup_{} failed on {}{}{}".format(clu_type, lc._RED, vm_name, lc._RESET))
    lc._level -= 1
...
def main():
    ...
    if not lc.validate_lab_definition(json_file, config, iso_loc, lab_setup_path, vm_img_loc=vm_img_loc):
        sys.exit(1)
