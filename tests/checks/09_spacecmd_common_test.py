...
fake = FakeSSH(responses=[
    ...
    ("test -f /etc/keycloak-tls", FakeResult(returncode=0)),  # cert already exists — skip openssl
    ("free -m", FakeResult(returncode=0, stdout="              total  used  free\nMem:            947   300   400\n")),
    ("podman run -d --name keycloak", FakeResult(returncode=0)),
    ("curl -sfk https://localhost:8080/realms/master", FakeResult(returncode=0)),
])
sc.ssh_run = fake
...
check("_keycloak_java_opts: uses a tight cap on a real ~900MB host (avoids the build-time OOM)",
      sc._keycloak_java_opts("neptune.mydemo.lab") == "-Xms128m -Xmx384m -XX:MaxMetaspaceSize=128m")

fake = FakeSSH(responses=[("free -m", FakeResult(returncode=0, stdout="Mem:  15792  552  14300\n"))])
sc.ssh_run = fake
check("_keycloak_java_opts: uses generous caps on a real 15GB+ host (avoids the runtime OOM)",
      sc._keycloak_java_opts("pluto.mydemo.lab") == "-Xms256m -Xmx1024m -XX:MaxMetaspaceSize=512m")

fake = FakeSSH(responses=[("free -m", FakeResult(returncode=1, stdout=""))])
sc.ssh_run = fake
check("_keycloak_java_opts: falls back to the generous caps if 'free -m' itself fails "
      "(never silently applies the wrong-for-abundant-hosts tight cap by default)",
      sc._keycloak_java_opts("mystery-host.mydemo.lab") == "-Xms256m -Xmx1024m -XX:MaxMetaspaceSize=512m")
check("ensure_keycloak: bind-mounts the TLS cert dir and sets the KC_HTTPS_CERTIFICATE_* env vars",
      any("-v /etc/keycloak-tls:/etc/keycloak-tls:ro" in c and
...
fake = FakeSSH(responses=[
    ("test -f /etc/keycloak-tls", FakeResult(returncode=1)),
    ("openssl req", FakeResult(returncode=0)),
])
sc.ssh_run = fake
sc._ensure_keycloak_tls_cert("neptune.mydemo.lab")
cmds = [c[1] for c in fake.calls]
...
sc.ssh_run = fake
sc.ensure_keycloak("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin")
check("ensure_keycloak: skips the run entirely when the container is already running and "
      "answers over HTTPS",
...
def _fake_running_but_broken(hostname, cmd, **kwargs):
    _seen_cmds.append(cmd)
    if "podman inspect -f" in cmd:
        _inspect_calls[0] += 1
        if _recreated[0]:
            return FakeResult(returncode=0, stdout="true\n")
        return FakeResult(returncode=0, stdout=("true\n" if _inspect_calls[0] == 1 else "false\n"))
    if "podman run -d --name keycloak" in cmd:
        _recreated[0] = True
        return FakeResult(returncode=0)
    if "curl -sfk https://localhost:8080/realms/master" in cmd:
        return FakeResult(returncode=0 if _recreated[0] else 1)
    return FakeResult(returncode=0)


sc.ssh_run = _fake_running_but_broken
sc.ensure_keycloak("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin")
check("ensure_keycloak: recreates a RUNNING container that fails the HTTPS readiness probe "
...
fake = FakeSSH(responses=[
    ("podman inspect -f", FakeResult(returncode=0, stdout="true\n")),
    ("curl -sfk https://localhost:8080/realms/master", FakeResult(returncode=1)),
])
sc.ssh_run = fake
...
sc.ensure_keycloak_saml_client("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                                ...
                                "https://sol.mydemo.lab/rhn/manager/sso/sls")
cmds = [c[1] for c in fake.calls]
...
fake = FakeSSH(responses=[
    ...
    ("get clients/abc-123-uuid/protocol-mappers/models", FakeResult(returncode=0, stdout=json.dumps([{
        "id": "mapper-id-1", "name": "uid", "config": {"attribute.name": "uid", "friendly.name": "uid"},
    }]))),
    ("update clients/abc-123-uuid/protocol-mappers/models/mapper-id-1", FakeResult(returncode=0)),
...
sc.ensure_keycloak_saml_client("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                                ...
                                "https://sol.mydemo.lab/rhn/manager/sso/acs")
cmds = [c[1] for c in fake.calls]
check("ensure_keycloak_saml_client: backfills (removes) a stale friendlyName on an EXISTING mapper",
      any("update clients/abc-123-uuid/protocol-mappers/models/mapper-id-1" in c and
          'config."friendly.name"=' in c for c in cmds))
check("ensure_keycloak_saml_client: does not try to recreate the client/mapper while backfilling it",
      not any("create clients" in c for c in cmds))

fake = FakeSSH(responses=[
    ("--fields id --format csv", FakeResult(returncode=0, stdout="abc-123-uuid")),
    ("get clients/abc-123-uuid/protocol-mappers/models", FakeResult(returncode=0, stdout=json.dumps([{
        "id": "mapper-id-1", "name": "uid", "config": {"attribute.name": "uid"},
    }]))),
    ("get clients -r", FakeResult(returncode=0, stdout="https://sol.mydemo.lab/rhn/manager/sso/metadata")),
])
sc.ssh_run = fake
sc.ensure_keycloak_saml_client("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                                ...
                                "https://sol.mydemo.lab/rhn/manager/sso/acs")
cmds = [c[1] for c in fake.calls]
check("ensure_keycloak_saml_client: leaves an already-correct existing mapper alone (no update call)",
      not any(c.startswith("update clients") for c in cmds))
...
fake = FakeSSH(responses=[
    ...
    ("get clients/abc-123-uuid/protocol-mappers/models",
     FakeResult(returncode=0, stdout='[{"name" : "uid"}]')),
    ("get clients -r", FakeResult(returncode=0, stdout=json.dumps([{
        "id": "abc-123-uuid", "clientId": "https://sol.mydemo.lab/rhn/manager/sso/metadata",
        "attributes": {},
    }]))),
    ("update clients/abc-123-uuid -r", FakeResult(returncode=0)),
])
sc.ssh_run = fake
sc.ensure_keycloak_saml_client("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                                "https://sol.mydemo.lab/rhn/manager/sso/metadata",
                                ...
                                "https://sol.mydemo.lab/rhn/manager/sso/sls")
cmds = [c[1] for c in fake.calls]
check("ensure_keycloak_saml_client: backfills a missing logout service URL on an EXISTING client",
      any("update clients/abc-123-uuid -r" in c and
          "attributes.saml_single_logout_service_url_redirect=https://sol.mydemo.lab/rhn/manager/sso/sls"
          in c for c in cmds))
check("ensure_keycloak_saml_client: does not try to recreate the client while backfilling its SLS url",
      not any("create clients" in c for c in cmds))
...
fake = FakeSSH(responses=[
    ...
    ("get clients -r", FakeResult(returncode=0, stdout=json.dumps([{
        "id": "abc-123-uuid", "clientId": "https://sol.mydemo.lab/rhn/manager/sso/metadata",
        "attributes": {"saml_single_logout_service_url_redirect":
                        "https://sol.mydemo.lab/rhn/manager/sso/sls"},
    }]))),
])
sc.ssh_run = fake
sc.ensure_keycloak_saml_client("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                                ...
                                "https://sol.mydemo.lab/rhn/manager/sso/sls")
cmds = [c[1] for c in fake.calls]
check("ensure_keycloak_saml_client: leaves an already-correct logout service URL alone",
      not any(c.startswith("update clients") for c in cmds))
...
fake = FakeSSH(responses=[("get users -r", FakeResult(returncode=0, stdout=json.dumps([{
    "id": "abc-123", "username": "brahe", "firstName": "Tycho", "lastName": "Brahe",
    "emailVerified": True,
}])))])
sc.ssh_run = fake
sc.ensure_keycloak_user("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                         "brahe", "Brahe12345", "brahe@mydemo.lab")
check("ensure_keycloak_user: skips entirely when the existing user already has all required "
      "profile fields set",
...
sc.ssh_run = fake
sc.ensure_keycloak_user("neptune.mydemo.lab", 8080, "lab-in-a-box", "admin", "admin",
                         "admin", "1234", "admin@mydemo.lab")
cmds = [c[1] for c in fake.calls]
check("ensure_keycloak_user: backfills missing firstName/lastName/emailVerified on an EXISTING "
      "user rather than leaving it permanently broken",
      any("update users/abc-123" in c and "firstName=admin" in c and "lastName=admin" in c and
          "emailVerified=true" in c for c in cmds))
check("ensure_keycloak_user: does NOT try to recreate an existing user while backfilling it",
      not any("create users" in c for c in cmds))
...
fake = FakeSSH(responses=[
    ("cat /etc/rhn/rhn.conf", FakeResult(returncode=0, stdout=_existing_5_stale)),
    ("mgradm restart", FakeResult(returncode=0)),
])
sc.ssh_run = fake
sc.ensure_sso("sol.mydemo.lab", "mgrctl exec --", {"smlm_sso": {"keycloak_host": "neptune.mydemo.lab"}}, "smlm")
cmds = [c[1] for c in fake.calls]
input_texts = [c[2].get("input_text") for c in fake.calls if c[2].get("input_text")]
written = input_texts[0] if input_texts else ""
check("ensure_sso: adds the missing x509cert key on a stale (pre-x509cert-fix) config",
      "java.sso.onelogin.saml2.idp.x509cert = FAKECERTDATA123==" in written)
check("ensure_sso: corrects stale http:// idp.entityid to https:// (the TLS fix)",
      "java.sso.onelogin.saml2.idp.entityid = https://neptune.mydemo.lab:8080/realms/lab-in-a-box"
      in written and "idp.entityid = http://neptune" not in written)
check("ensure_sso: corrects stale http:// idp.single_sign_on_service.url to https:// too",
      "java.sso.onelogin.saml2.idp.single_sign_on_service.url = "
      "https://neptune.mydemo.lab:8080/realms/lab-in-a-box/protocol/saml" in written)
check("ensure_sso: leaves the unaffected sp.* keys untouched",
      "java.sso.onelogin.saml2.sp.entityid = https://sol.mydemo.lab/rhn/manager/sso/metadata" in written)
...
_existing_7_correct = "\n".join([
    ...
    "java.sso.onelogin.saml2.idp.entityid = https://neptune.mydemo.lab:8080/realms/lab-in-a-box",
    "java.sso.onelogin.saml2.idp.single_sign_on_service.url = "
    "https://neptune.mydemo.lab:8080/realms/lab-in-a-box/protocol/saml",
    "java.sso.onelogin.saml2.idp.single_logout_service.url = "
    "https://neptune.mydemo.lab:8080/realms/lab-in-a-box/protocol/saml",
    "java.sso.onelogin.saml2.idp.x509cert = FAKECERTDATA123==",
]) + "\n"
fake = FakeSSH(responses=[("cat /etc/rhn/rhn.conf", FakeResult(returncode=0, stdout=_existing_7_correct))])
sc.ssh_run = fake
sc.ensure_sso("sol.mydemo.lab", "mgrctl exec --", {"smlm_sso": {"keycloak_host": "neptune.mydemo.lab"}}, "smlm")
cmds = [c[1] for c in fake.calls]
check("ensure_sso: fully idempotent when all 7 keys already exist with correct values — "
      "no write, no restart",
      not any(c.startswith("cat >") for c in cmds) and not any("mgradm restart" in c for c in cmds))

sc.ssh_run = FakeSSH(responses=[("cat /etc/rhn/rhn.conf", FakeResult(returncode=1, stdout=""))])
died = False
try:
    sc.ensure_sso("sol.mydemo.lab", "mgrctl exec --",
                  {"smlm_sso": {"keycloak_host": "neptune.mydemo.lab"}}, "smlm")
except SystemExit:
    died = True
...
sc.ssh_run = FakeSSH(responses=[
    ("cat /etc/rhn/rhn.conf", FakeResult(returncode=0, stdout="# empty\n")),
    ("mgradm restart", FakeResult(returncode=0)),
])
sc.ensure_sso("sol.mydemo.lab", "mgrctl exec --", {
    "smlm_admin_user": "admin", "smlm_admin_pass": "Smlm12345",
    "smlm_users": [
        {"username": "brahe", "password": "Brahe12345", "email": "brahe@mydemo.lab"},
        {"username": "keplerpam", "password": "Kepler12345", "pam": True},
        {"username": "incomplete-no-password"},
    ],
    "smlm_sso": {"keycloak_host": "neptune.mydemo.lab", "demo_user": "extra-demo",
                 "demo_password": "ExtraDemo123"},
}, "smlm")
created_usernames = [u for u, _, _ in _created_kc_users]
check("ensure_sso: creates a Keycloak user for the real SMLM admin account (the actual live bug)",
      "admin" in created_usernames)
check("ensure_sso: creates a Keycloak user for every real smlm_users entry with a password",
      "brahe" in created_usernames)
check("ensure_sso: skips a 'pam': true smlm_users entry (authenticates against the OS, not SAML)",
      "keplerpam" not in created_usernames)
check("ensure_sso: skips an smlm_users entry with no password to create a Keycloak account from",
      "incomplete-no-password" not in created_usernames)
check("ensure_sso: still creates the explicit extra demo_user too",
      "extra-demo" in created_usernames)
...
sc.provision_virtual_guest("host1", "mgrctl exec --", "nuc6.mydemo.lab", "vguest1", "sles15sp7-example-ks",
                            2048, 2, 20)
cmds = [c[1] for c in fake.calls]
create_cmd = next(c for c in cmds if "system.provisionVirtualGuest" in c)
...
fake = FakeSSH(responses=[
    ("system.getId", FakeResult(returncode=0, stdout=json.dumps([{"id": 42, "name": "nuc6.mydemo.lab"}]))),
    ("system.addEntitlements", FakeResult(returncode=1, stderr="not a virtualization-capable system")),
])
sc.ssh_run = fake
died = False
try:
    ...
    died = True
check("provision_virtual_guest: dies with a clear message if the entitlement can't be enabled", died)

fake = FakeSSH(responses=[
    ("system.getId", FakeResult(returncode=0, stdout=json.dumps([{"id": 42, "name": "nuc6.mydemo.lab"}]))),
    ("system.addEntitlements", FakeResult(returncode=0)),
